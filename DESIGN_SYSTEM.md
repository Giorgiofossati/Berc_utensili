...
**`max-md:h-11` su `SelectTrigger` non basta da solo** *(bug reale osservato, 2026-09-24)*: il componente base `SelectTrigger` (`src/components/ui/select.jsx`) porta di suo `data-[size=default]:h-8` (32px). Quell'utility ha specificità CSS più alta di una classe responsive semplice come `max-md:h-11` (selettore attributo + classe batte una classe sola, indipendentemente dall'ordine nel markup o da `cn()`/tailwind-merge, che non riconosce i due come in conflitto perché hanno modificatori diversi) — quindi il trigger resta a 32px anche quando il codice sembra correttamente impostato a 44px su mobile. Misurato dal vivo su `DropdownFilterView` e `ToolsGrid`: 32px invece di 44px finché non corretto. **Fix**: forzare l'override con l'important modifier di Tailwind v4, `max-md:h-11!` (il trailing `!`, non `!` davanti alla classe). Un `min-h-[44px]` accanto a un `h-*` in conflitto funziona ugualmente (vince perché `min-height` non è toccato da `data-[size=default]`, come già in `AddToolModal`), ma il modifier `!` è la via esplicita quando si aggiunge solo un'altezza. **Nota di metodo**: qualsiasi override di dimensione su un componente con varianti `data-[size=...]` (oggi solo `select.jsx`) va verificato con `getComputedStyle` nel browser, non solo letto nel codice — il markup può mentire.
...
| `xl` | 1024px | Selezione tabellare estesa (picker distinta) | `ToolsGrid` integrata |
...
**L'altezza del modale va limitata nel suo insieme, non solo nel corpo** *(bug reale osservato, 2026-09-24)*: `DialogContent` (`src/components/ui/dialog.jsx`) era `display: grid` senza un limite di altezza complessivo; solo `ModalBody` aveva `max-h-[85vh]`. Risultato: su un viewport mobile realistico (es. 375×650, con la barra degli indirizzi visibile — non i 375×812 "a schermo pieno" con cui si testa di solito) un modale con header + corpo + footer alti quanto basta (es. Dettaglio Utensile, Modifica Commessa, Dettaglio Movimento — tutti costruiti sugli stessi `ModalHeader`/`ModalBody`/`ModalFooter`) superava facilmente il 100% del viewport. Essendo centrato con `top-1/2 -translate-y-1/2` e senza limite, la parte in eccesso finiva sopra y=0 o sotto il fondo dello schermo — **senza che la pagina potesse scorrere per recuperarla**: i bottoni del footer (DEPOSITA/PRELEVA, SALVA/Elimina) diventavano completamente irraggiungibili, non solo tagliati alla vista. Misurato dal vivo: popup di 826px su un viewport di 650px, footer a y=768 (fuori schermo), `document.documentElement.scrollHeight === clientHeight` (pagina non scorribile). **Fix**: `DialogContent` è ora `flex flex-col max-h-[calc(100dvh-2rem)]` (non `vh`: su mobile reale `vh` conta lo spazio dietro la barra degli indirizzi anche quando non è visibile, `dvh` segue l'altezza vista davvero); `ModalHeader`/`ModalFooter` restano `shrink-0`; `ModalBody` è `flex-1 min-h-0 overflow-y-auto` invece di `max-h-[85vh]` fisso — così il corpo, non header/footer, cede lo spazio quando il contenuto non ci sta, e i bottoni restano sempre visibili e raggiungibili. **Nota di metodo**: questo tipo di bug non si vede testando a viewport "pieno" (375×812 senza chrome del browser) — va verificato anche a un'altezza ridotta (es. 375×650) per simulare la barra degli indirizzi/strumenti di un browser mobile reale.
