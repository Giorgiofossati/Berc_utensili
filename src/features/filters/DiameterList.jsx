    ...
    <div className="max-h-[50vh] overflow-y-auto custom-scrollbar p-2.5 sm:p-3 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2">
      {diameters.map((d, i) => (
        <motion.button
          ...
          onClick={() => onSelect(d)}
          className="glass-button rounded-[12px] sm:rounded-[16px] px-2 py-2.5 sm:px-4 sm:py-3.5 app-qty-sm text-center hover:bg-accent-blue/10 hover:border-accent-blue/30 hover:text-accent-blue active:scale-95 transition-all"
        >
          Ø{d}
        </motion.button>
      ))}
