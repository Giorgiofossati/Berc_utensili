...
const OrderModal = ({ tool, onClose, currentUser, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNaN(qtyNum) || qtyNum <= 0) {
      setErrorMessage("La quantità richiesta deve essere un numero intero maggiore di zero.");
      return;
    }
    ...
    try {
      ...
      setTimeout(() => {
        if (onSuccess) {
          onSuccess(`Ordine di riassortimento creato: ${qtyNum} pz`);
        }
        onClose();
      }, 1500);
    } catch (err) {
      console.error(err);
      setErrorMessage(err.message || "Errore durante la creazione dell'ordine");
    } finally {
      setIsLoading(false);
    }
  };
