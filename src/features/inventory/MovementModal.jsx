              ...
              <motion.div key="details" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-3 md:gap-4 pb-2">
                ...
                <div className={`grid ${(!isBulkMode && selectedTool && (selectedTool?.['Quantità'] || 0) <= 0 && currentUser?.ruolo !== 'Admin') ? 'grid-cols-1' : 'grid-cols-2 lg:grid-cols-3'} gap-2 sm:gap-3 mt-1`}>
                  {(!(!isBulkMode && selectedTool && (selectedTool?.['Quantità'] || 0) <= 0 && currentUser?.ruolo !== 'Admin')) && (
                    <>
                      <button onClick={() => setOpType('carico')} className="action-btn action-btn-carica py-2.5 sm:py-3 md:py-3.5 w-full flex flex-col items-center justify-center gap-1 group border border-accent-emerald/30 shadow-lg relative overflow-hidden rounded-xl sm:rounded-2xl active:scale-95">
                        ...
                        <span className="app-overline relative z-10">DEPOSITA</span>
                      </button>
                      <button onClick={() => setOpType('scarico')} className="action-btn action-btn-scarica py-2.5 sm:py-3 md:py-3.5 w-full flex flex-col items-center justify-center gap-1 group border border-accent-rose/30 shadow-lg relative overflow-hidden rounded-xl sm:rounded-2xl active:scale-95">
                        ...
                        <span className="app-overline relative z-10">PRELEVA</span>
                      </button>
                    </>
                  )}
                  {!isBulkMode && (
                    <button onClick={() => { setShowMoveModal(false); if(onOpenOrder) onOpenOrder(); }} className={`action-btn py-2.5 sm:py-3 md:py-3.5 w-full flex flex-col items-center justify-center gap-1 group border border-accent-orange/30 shadow-lg relative overflow-hidden rounded-xl sm:rounded-2xl active:scale-95 ${(!(!isBulkMode && selectedTool && (selectedTool?.['Quantità'] || 0) <= 0 && currentUser?.ruolo !== 'Admin')) ? 'col-span-2 lg:col-span-1' : ''}`} style={{ backgroundColor: 'rgba(249, 115, 22, 0.05)' }}>
                      ...
                      <span className="app-overline relative z-10 text-accent-orange text-center">CREA ORDINE</span>
                    </button>
                  )}
                </div>
              </motion.div>
            ) : (
              <motion.div key="operation" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex flex-col h-full justify-center gap-4 sm:gap-6 md:gap-8 my-auto py-3">
                
                <div className="flex items-center gap-3 sm:gap-6 md:gap-8 justify-center">
                  <button onClick={() => setModalQty(Math.max(1, modalQty - 1))} className="w-11 h-11 sm:w-14 sm:h-14 md:w-16 md:h-16 glass-button rounded-full text-lg sm:text-xl md:text-2xl font-black shrink-0 hover:bg-white/10 hover:scale-110 active:scale-95 transition-all">-</button>
                  <div className="flex flex-col items-center">
                    <p className="app-overline text-accent-orange mb-1 text-center">Quantità {opType === 'carico' ? 'da Depositare' : 'da Prelevare'}</p>
                    <input type="number" inputMode="numeric" value={modalQty} onChange={(e) => setModalQty(parseInt(e.target.value) || 1)} className="w-20 sm:w-24 md:w-28 bg-transparent text-center text-3xl sm:text-4xl md:text-5xl font-black outline-none dark:text-white text-slate-900 tabular-nums border-b-2 dark:border-white/10 border-slate-900/10 pb-1 focus:border-accent-blue transition-all" />
                  </div>
                  <button onClick={() => setModalQty(modalQty + 1)} className="w-11 h-11 sm:w-14 sm:h-14 md:w-16 md:h-16 glass-button rounded-full text-lg sm:text-xl md:text-2xl font-black shrink-0 hover:bg-white/10 hover:scale-110 active:scale-95 transition-all">+</button>
                </div>

                <div className="flex items-center gap-3 md:gap-4 w-full mt-2">
                  {!isBulkMode && (
                    <button onClick={() => setOpType(null)} className="glass-button px-4 sm:px-6 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl dark:text-slate-400 text-slate-600 hover:dark:text-white text-slate-900 w-auto shrink-0 flex items-center justify-center transition-all group">
                       <span className="app-overline group-hover:-translate-x-1 transition-transform">Indietro</span>
                    </button>
                  )}
                  <button
                    onClick={onConfirm}
                    className={`flex-1 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl app-overline tracking-[0.2em] shadow-2xl hover:scale-[1.02] active:scale-[0.98] transition-all ${opType === 'carico' ? 'bg-accent-emerald text-slate-950 border-emerald-400/50 hover:bg-emerald-400' : 'bg-accent-rose text-white border-rose-500/50 hover:bg-rose-500'}`}
                  >
                    CONFERMA
                  </button>
                </div>
