      ...
      <DialogContent showCloseButton={false} className="glass-panel w-[95vw] sm:w-full max-w-lg p-6 md:p-10 rounded-[40px] z-[2001] bg-transparent border-none shadow-none overflow-hidden focus:outline-none">
        ...
        {!selectedUser ? (
            ...
            <div className="flex flex-col gap-2 max-h-[400px] overflow-y-auto p-1 pb-4 pr-2 custom-scrollbar">
              {loading ? (
                <div className="py-10 text-center flex flex-col items-center gap-4">
                  ...
                  <p className="dark:text-white text-slate-900/40 text-sm">Caricamento operatori...</p>
                </div>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-left">
                        <p className="font-bold dark:text-white text-slate-900 uppercase tracking-tight">{user.nome} {user.cognome}</p>
                        <p className="text-xs dark:text-white text-slate-900/40 font-mono tracking-widest">{user.codice_id}</p>
                      </div>
                    </div>
                ...
                <div className="py-10 text-center flex flex-col items-center gap-4 dark:bg-white/5 bg-slate-900/5 rounded-3xl p-8 border border-white/5">
                  ...
                  <div>
                    <p className="dark:text-white text-slate-900 font-bold mb-1">Nessun operatore trovato</p>
                    <p className="dark:text-white text-slate-900/40 text-xs max-w-[250px]">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            <div className="flex items-center gap-4 mb-8 p-4 dark:bg-white/5 bg-slate-900/5 rounded-2xl border dark:border-white/10 border-slate-900/10">
              ...
              <div className="flex-1">
                ...
                <h4 className="text-xl font-black dark:text-white text-slate-900 uppercase">{selectedUser.nome} {selectedUser.cognome}</h4>
              </div>
              <button onClick={() => setSelectedUser(null)} className="text-xs dark:text-white text-slate-900/40 hover:dark:text-white text-slate-900 underline uppercase p-2">Cambia</button>
            </div>
