...
- **Griglia Intelligente a Tessere per Diametri / Sigle (`DiameterList.jsx`) (Completata):** Selettore di livello 2 trasformato in griglia auto-adattiva a tessere responsive (`auto-fill / minmax`). Prefisso `Ø` applicato solo a valori strettamente numerici (sigle come `CCMT`, `D1.5`, `APKT` renderizzate pulite con badge `SIGLA`), badge istantaneo di giacenza a magazzino (`totalQty` in verde per pezzi > 0, rosso per 0 pz), contatore articoli e micro-barra di ricerca rapida istantanea con pulizia filtro rapida (`X`) ed empty state dedicato.
